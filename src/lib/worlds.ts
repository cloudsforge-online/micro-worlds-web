 ...
 * (`worlds/src/server.ts:362-838`), so the surface is enumerable and the citations are stable.
 * `test/worlds.test.ts` reads that file and fails if any line below is not the line that registers
 * the route, and CI bends a citation to prove the check can go red.
 *
 * ── CALLED ────────────────────────────────────────────────────────────────────────────────────
 *
 * | Method   | Path                                | Authenticates | Verified at                |
 * | -------- | ----------------------------------- | ------------- | -------------------------- |
 * | `GET`    | `/v1/titles`                        | **no**        | `worlds/src/server.ts:467` |
 * | `GET`    | `/v1/players/me`                    | yes           | `worlds/src/server.ts:524` |
 * | `PUT`    | `/v1/players/me`                    | yes           | `worlds/src/server.ts:551` |
 * | `PUT`    | `/v1/players/me/cosmetics`          | yes           | `worlds/src/server.ts:576` |
 * | `GET`    | `/v1/players/me/inventory`          | yes           | `worlds/src/server.ts:598` |
 * | `POST`   | `/v1/players/me/inventory/:id/list` | yes           | `worlds/src/server.ts:617` |
 * | `DELETE` | `/v1/players/me/inventory/:id/list` | yes           | `worlds/src/server.ts:631` |
 * | `GET`    | `/v1/provisions`                    | yes           | `worlds/src/server.ts:642` |
 * | `GET`    | `/v1/provisions/:id`                | yes           | `worlds/src/server.ts:683` |
 * | `GET`    | `/v1/titles/:id/achievements`       | **no**        | `worlds/src/server.ts:701` |
 * | `GET`    | `/v1/titles/:id/seasons`            | **no**        | `worlds/src/server.ts:755` |
 *
 ...
 * | ------ | ------------------------------------- | -------------------------- | ------------ |
 * | `POST` | `/v1/events`                          | `worlds/src/server.ts:389` | The entitlement bridge's front door. It is HMAC-checked over the exact bytes received BEFORE `JSON.parse` (`server.ts:392-398`) with `OUTBOX_SIGNING_SECRET`. A browser cannot hold that key, and a bundle that shipped it would BE the free-worlds endpoint the check exists to prevent. |
 * | `POST` | `/v1/titles`                          | `worlds/src/server.ts:484` | Registering a title says where a customer's purchase will be sent. `server.ts:488-489` demands `worlds:admin` or `role:admin`. See "the empty registry" below — this is the missing piece, and it is missing from the ESTATE, not from this app. |
 * | `POST` | `/v1/provisions/:id/retry`            | `worlds/src/server.ts:667` | `server.ts:669-670` demands `worlds:admin` or `role:admin`. It is the only way out of `failed` (`server.ts:663-665`), and the service's own comment at `server.ts:645-646` says no view of failed rentals exists anywhere in the estate. That view belongs in the operator console; a player product growing an operator's escape hatch is how the console ends up in six repositories. Reported, not built here. |
 * | `PUT`  | `/v1/titles/:id/achievements`         | `worlds/src/server.ts:714` | `worlds:title` or `role:admin` (`server.ts:718-719`). A title service defines its own achievements. |
 * | `POST` | `/v1/titles/:id/achievements/unlock`  | `worlds/src/server.ts:735` | `worlds:title` or `role:admin` (`server.ts:737-738`). A title reports what a player did; a player does not report it about themselves. |
 * | `POST` | `/v1/titles/:id/seasons`              | `worlds/src/server.ts:760` | Opening a season sets a MONEY budget, so `server.ts:764-765` demands admin. `server.ts:762-763`: "A title that could set its own reward budget could pay itself." |
 * | `GET`  | `/v1/seasons/:id/budget`              | `worlds/src/server.ts:779` | A season's remaining reward budget is an operator's number, not a player's, and rendering "1,412 Shards left in the pot" to a player is an invitation to race for it. It is also the one worlds route the gateway does not route: `deploy/gateway/dynamic/public-api.yml:143` matches `/v1/titles`, `/v1/players` and `/v1/provisions` and not `/v1/seasons`, so it falls to the catch-all at `:151` and is blackholed to `127.0.0.1:1` (`:196-198`). Reported. |
 * | `POST` | `/v1/seasons/:id/rewards`             | `worlds/src/server.ts:802` | `worlds:title` or `role:admin` (`server.ts:804-805`). A title ASKS for a reward; it does not decide. Paying oneself from a browser is the exploit the budget exists to bound. |
 *
 ...
 *
 * `GET /v1/titles` (`:467`), `GET /v1/titles/:id/achievements` (`:701`) and
 * `GET /v1/titles/:id/seasons` (`:755`) contain no `await authenticate(ctx, deps)` at all. Their
 ...
 * middleware and no wrapper: `handle()` (`server.ts:284-353`) dispatches straight into the route's
 ...
 * (`worlds/src/players.ts:424`), and the bridge's own idempotency key is the entitlement id, which
 ...
 * access — never power. `worlds/src/players.ts:390-391` carries the same rule from
 ...
 * (`worlds/src/server.ts:307-312`), backstopped by a WHERE clause and a CHECK constraint.
 *
 * `bound` is on the wire precisely so a client knows before it draws a control
 * (`worlds/src/server.ts:861-862`). This app therefore never renders an entitlement as an
...
/** `worlds/src/titles.ts:26` — five, and the app must render all of them. */
export const TITLE_STATUSES = ['draft', 'beta', 'live', 'sunset', 'retired'] as const
export type TitleStatus = (typeof TITLE_STATUSES)[number]

/**
 * What a title declares it can be asked to do — `worlds/src/titles.ts:43`.
 *
 * A CLOSED set, and `worlds/src/titles.ts:39-41` says why: these are read by the provisioning
...
/** One registry row, as `GET /v1/titles` puts it on the wire — `worlds/src/server.ts:472-479`. */
...
/**
 * A title that may be sold to — `worlds/src/titles.ts:227-229`.
 *
...
/** The cross-title key in `equippedCosmetics` — `worlds/src/players.ts:216`. */
export const CROSS_TITLE = '*'

/** `worlds/src/players.ts:24` — an age bracket is a safeguarding fact, not a preference. */
export const AGE_BRACKETS = ['unknown', 'under_13', '13_to_15', '16_to_17', 'adult'] as const
export type AgeBracket = (typeof AGE_BRACKETS)[number]

/** `worlds/src/players.ts:45-51`. `scope` is a title id, or `*` when estate-wide. */
...
/**
 * The account-scoped profile — `worlds/src/players.ts:53-64`, serialised straight to the wire by
 * `GET /v1/players/me` (`worlds/src/server.ts:537`) with no `toWire`, so its `Date` fields arrive
 * as ISO strings.
 *
 * `equippedCosmetics` is keyed BY TITLE, and `worlds/src/players.ts:13-18` explains at length why
...
/** `worlds/src/players.ts:201`. Five ways an item arrives in an account. */
export const ITEM_SOURCES = ['purchase', 'reward', 'craft', 'market', 'grant'] as const
export type ItemSource = (typeof ITEM_SOURCES)[number]

/** One inventory row, as `toItemWire` puts it on the wire — `worlds/src/server.ts:855-867`. */
export interface InventoryItem {
  ...
  /**
   * **The anti-pay-to-win control** (`worlds/src/players.ts:242`). On the wire "because a client
   * that offers a 'sell' button must know before it draws one" (`worlds/src/server.ts:861-862`).
   */
...
/** `GET /v1/players/me`'s achievement rows — `worlds/src/server.ts:540-546`. */
...
/** `worlds/src/rewards.ts` — as `GET /v1/titles/:id/achievements` renders it (`server.ts:706-710`). */
...
/**
 * One season, as `toSeasonWire` puts it on the wire — `worlds/src/server.ts:870-893`.
 *
 * Both Shard fields are STRINGS, always, and `worlds/src/server.ts:889` says why in three words:
 * "A budget is money."
 */
...
/** `worlds/src/provisioning.ts:66-71`. What a SKU means, resolved by a table not a heuristic. */
...
/**
 * `worlds/src/provisioning.ts:73-79`. Five states, and the app must render all five.
 *
 * `unsupported` is the one this product exists to be honest about: it is a customer who paid for
 * something undeliverable. `worlds/src/server.ts:129` says so in the metric's own help text.
 */
...
/**
 * A provision, as `worlds/src/provisioning.ts:124-140` defines it. Serialised straight to the wire
 * by `GET /v1/provisions` (`worlds/src/server.ts:651`) with no `toWire`, so its `Date` fields
 ...
 * (`worlds/src/provisioning.ts:429`, `:437-449`, `:466`) — and this app renders it verbatim rather
...
/**
 * `GET /v1/titles` — `worlds/src/server.ts:467`.
 *
 ...
 * `'true'` turns it on (`server.ts:468`). Retired titles are excluded by default at
 * `worlds/src/titles.ts:200-202`.
 */
...
/**
 * `GET /v1/players/me` — `worlds/src/server.ts:524`.
 *
 ...
 * (`server.ts:537`). It is not an error and it is not a loading state: it is a new player. The
 * type says so, and the screen renders it as an invitation rather than as a blank.
 *
 * **This route FAILS OPEN**, deliberately — `worlds/src/server.ts:520-522`: it runs on every app
...
/**
 * `PUT /v1/players/me` — `worlds/src/server.ts:551`.
 *
 ...
 *
 * `ageBracket` and `parentalControls` are accepted by the handler (`server.ts:559-564`) and are
 * deliberately NOT offered by this app: an age bracket is a safeguarding fact
 * (`worlds/src/players.ts:8-11`), and a screen that lets an account assert its own is a screen
...
/**
 * `PUT /v1/players/me/cosmetics` — `worlds/src/server.ts:576`.
 *
 ...
 * `entitlements_unavailable` (`server.ts:341-348`) and this app renders that as its own sentence
 * rather than as a generic failure.
 *
 * `itemUrn: null` CLEARS the slot, and the handler skips the entitlement check for it
 * (`server.ts:585-590`): "you may always take something off, including something you no longer
...
/**
 * `GET /v1/players/me/inventory` — `worlds/src/server.ts:598`.
 *
...
/**
 * `POST /v1/players/me/inventory/:id/list` — `worlds/src/server.ts:617`.
 *
 ...
 *
 * 201 on success. A row already listed is a 409 `inventory_state` (`server.ts:321-323`).
 */
...
/**
 * `DELETE /v1/players/me/inventory/:id/list` — `worlds/src/server.ts:631`.
 *
...
/**
 * `GET /v1/provisions` — `worlds/src/server.ts:642`.
 *
 ...
 * whose subject is their own user (`server.ts:653-658`). This app sends no subject and cannot: it
 * is derived from the token.
 *
 * `state` is passed through verbatim to the query (`server.ts:644`), so it is only ever sent from
...
/**
 * `GET /v1/provisions/:id` — `worlds/src/server.ts:683`.
 *
...
/**
 * `GET /v1/titles/:id/achievements` — `worlds/src/server.ts:701`.
 *
...
/**
 * `GET /v1/titles/:id/seasons` — `worlds/src/server.ts:755`.
 *
 ...
 *
 *   * `GET /v1/title`     — `worlds/src/titleclient.ts:122`, inside `describe()`
 *   * `POST /v1/provision` — `worlds/src/titleclient.ts:134-135`, inside `provision()`
 *
 * **Neither `micro-emberkin` nor `micro-nda` serves either one.** Both route tables were read:
 * `emberkin/src/server.ts` registers ten routes (`:238`-`:431`) and `nda/src/server.ts` registers
 * thirty-two across `define` (`:409`-`:654`) and `defineMutation` (`:431`-`:965`); no `/v1/title`
 ...
 * (`worlds/src/provisioning.ts:441-451`): "Asked BEFORE the call rather than discovered from a
...
export const TITLE_BRIDGE_GAP: KnownGap = {
  ...
  citations: [
    'worlds/src/titleclient.ts:122',
    'worlds/src/titleclient.ts:134-135',
    'worlds/src/provisioning.ts:441-451',
    'emberkin/src/server.ts:238-431',
    'nda/src/server.ts:409-965',
  ],
  closes:
...
export const EMPTY_REGISTRY_GAP: KnownGap = {
  ...
  citations: ['worlds/src/server.ts:467', 'worlds/src/server.ts:484-489', 'worlds/src/titles.ts:117-122'],
  closes:
